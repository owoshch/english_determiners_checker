...
class NERModel(LanguageModel):
  ...
  def load_embeddings(self):
    ...
    #word_to_num = self.config.vocab_words
    #num_to_word = {v:k for k,v in word_to_num}
    #tag_to_num = self.config.vocab_tags
    print (self.config.vocab_tags)
  ...
  def load_data(self, debug=False):
    ...
    print (tag_to_num)
    self.num_to_tag = {v:k for k,v in tag_to_num.items()}
    '''
...
def test_NER():
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as session:
      ...
      saver.restore(session, self.config.model)
      print 'Test'
      print '=-=-='
