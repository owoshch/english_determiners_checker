...
def input_to_windows(seq, word_to_num, tag_to_num, left=1, right=1, wsize=3):
    X = []
    words = seq.split()
    pad = (wsize - 1)/2
    words = pad * ["<s>"] + words + pad * ["</s>"]
    #print ('words', words)
    #preprocessed_words = [config.processing_word(w) for w in words_raw]
...
def to_windows(words, word_to_num, tag_to_num, wsize=3):
    pad = (wsize - 1)/2
    words = pad * ["<s>"] + words + pad * ["</s>"]
